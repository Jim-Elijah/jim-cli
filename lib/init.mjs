...
async function main() {
  ...
  const templateNameQuestion = {
    ...
    choices: entries.map(([name, url]) => ({ name, value: name, description: `url of "${name}" is "${url}"` })),
  }
  ...
  try {
    ...
    spinner.succeed();
    console.log(chalk.green(symbols.success), chalk.green(`Generating ${chalk.yellow(projectName)} completed!`))
    console.log('\n To get started')
