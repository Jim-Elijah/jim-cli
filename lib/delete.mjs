...
async function main() {
  ...
  const searchQuestion = {
    ...
    source: async (input) => {
      const query = inputTrimFilter(input || '');
      if (!query) {
      ...
      return entries
        .filter(([name, url]) => name.includes(query) || url.includes(query))
        .map(([name, url]) => ({ name, value: name, description: `url of "${name}" is "${url}"` }));
    },
  };
  ...
  try {
    ...
    console.log(
      chalk.green(symbols.success, `Delete template ${chalk.yellow(name)} successfully!\n`)
    );
    await showTemplate();
  } catch (err) {
